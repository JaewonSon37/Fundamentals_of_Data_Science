...
dummies <- dummies[, colSums(var(dummies) > 0)]
head(dummies)
...
zero_variance_vars <- names(dummies)[apply(dummies, 2, var) == 0]
dummies_processed <- dummies[, apply(dummies, 2, var) > 0]
preProc <- preProcess(dummies_processed, method = "pca", pcaComp = 2)
