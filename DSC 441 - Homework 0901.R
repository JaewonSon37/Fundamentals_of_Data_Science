...
set.seed(37)
fit <- kmeans(clustering_exoplantes, centers = 2, nstart = 20)
...
## Add column
rotated_data$Type <- final_exoplanets$planet_type
...
decision_tree <- train(group ~ ., data = classification_exoplanet,
...
svm <- train(group ~ ., data = classification_exoplanet,
...
pred_decision_tree <- predict(decision_tree, classification_exoplanet)
confusionMatrix(classification_exoplanet$group, pred_decision_tree)
...
pred_prob <- predict(decision_tree, classification_exoplanet, type = "prob")
roc_curve <- roc((classification_exoplanet$group), pred_prob[ , 1])
